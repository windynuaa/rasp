...
$(document).ready(function() {
    ...
    var chartRAM_real = Highcharts.chart('container-disk3', Highcharts.merge(gaugeOptions, {
        yAxis: {
            min: 0,
            max: init_vals.disk3.total,
            title: {
                text: ''
            }
        },
    ...
    var chartSWAP = Highcharts.chart('container-disk2', Highcharts.merge(gaugeOptions, {
        yAxis: {
            min: 0,
            max: init_vals.disk2.total,
            title: {
                text: ''
            }
        },
    ...
    setInterval(function() {
	    ...
	    $("#disk2-percent").text(data.disk2.percent);
            $("#disk2-free").text(data.disk2.free);
	    $("#disk3-percent").text(data.disk3.percent);
            $("#disk3-free").text(data.disk3.free);
            $("#loadavg-1m").text(data.load_avg[0]);
            $("#loadavg-5m").text(data.load_avg[1]);
            $("#loadavg-10m").text(data.load_avg[2]);
	    $("#cpu-freq").text(data.cpu.freq/1000);
        ...
        if(window.dashboard != null){
            ...
            if (chartRAM_real) {
                ...
                point.update(window.dashboard.disk3.used);
            }
            if (chartSWAP) {
                point = chartSWAP.series[0].points[0];
                point.update(window.dashboard.disk2.used);
            }
            if (chartDisk) {
